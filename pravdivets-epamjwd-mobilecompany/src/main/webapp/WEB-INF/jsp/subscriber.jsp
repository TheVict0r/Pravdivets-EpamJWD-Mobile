...
<body class="d-flex flex-column min-vh-100 bg-light">
	...
	<div
		...
		<table class="table">
				...
				<c:when test="${sessionScope.activate_edit eq 'phone'}">
					...
					<tr class="fw-normal table-secondary ">
						...
						<td colspan="2"><fmt:message
								key="subscriber.change-phone-confirm" /></td>
					</tr>
				...
				<c:choose>
					<c:when test="${sessionScope.activate_edit eq 'personal_data'}">
						<tr class="fw-normal table-secondary">
							...
							<td></td>
						<tr class="fw-normal table-secondary">
							<td><label for="subscriber_first_name" class="form-label"><fmt:message
						...
						<tr class="fw-normal table-secondary">
							<td><label for="subscriber_middle_name" class="form-label"><fmt:message
										key="add-subscriber.middle-name" />:</label></td>
							<td><input type="text" class="form-control"
								name="subscriber_middle_name" id="subscriber_middle_name"
								pattern="^([А-Я]{1}[а-яё-]+|[A-Z]{1}[a-z-]+)$"
						...
						<tr class="fw-normal table-secondary">
							...
							<td><input type="text" class="form-control" name="passport"
								id="passport" value="${sessionScope.subscriber_user.passport}"
								required></td>
							<td></td>
						</tr>
						<tr class="fw-normal table-secondary">
			...
			<c:choose>
				<c:when test="${sessionScope.activate_edit eq 'email'}">
					<form method="post" action=controller?command=edit_email>
					<tr class="fw-normal table-secondary">
				...
				<c:otherwise>
					<tr>
						...
						<td><a class="login btn btn-outline-primary btn-sm"
							href="${pageContext.request.contextPath}/controller?command=edit_email_preparation"><fmt:message
									key="subscriber.change" /></a></td>
					</tr>
				</c:otherwise>
			</c:choose>
			...
			<tr>
				...
				<td><a class="btn btn-outline-primary btn-sm"
					href="${pageContext.request.contextPath}/controller?command=go_to_subscriber_bills_page">
						<fmt:message key="subscriber.bills" />
				</a></td>
			</tr>
					...
					<c:when test="${sessionScope.activate_edit eq 'status'}">
						...
						<tr class="fw-normal table-secondary">
							...
							<td><select class="form-select form-select-sm"
								id="new_status" name="new_status">
									<option value="0">
										<fmt:message key="subscriber.status-active" />
									</option>
									<option value="1">
										<fmt:message key="subscriber.status-semi-blocked" />
									</option>
									<option value="2">
										<fmt:message key="subscriber.status-blocked" />
									</option>
									<option value="3">
										<fmt:message key="subscriber.status-deactivated" />
									</option>
									<option value="4">
										<fmt:message key="subscriber.status-paused" />
									</option>
							</select></td>
							<td></td>
						</tr>
			...
			<tr>
				...
				<td><a class="btn btn-outline-dark btn-sm"
					href="${pageContext.request.contextPath}/controller?command=go_to_subscriber_services_page">
						<fmt:message key="subscriber.services-page" />
				</a></td>
				<td></td>
			</tr>
		</table>
		<c:if test="${sessionScope.role != 'SUBSCRIBER'}">
			<a class="btn btn-primary btn-sm"
				...
				<fmt:message key="subscriber.operations-page" />
			</a>
		</c:if>
	</div>
